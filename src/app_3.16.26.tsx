...
function calcConnPrice(c) {
  ...
  if (c <= 25) return c*29; if (c <= 50) return c*22; if (c <= 100) return c*19;
  if (c <= 200) return c*17; if (c <= 300) return c*15.3; return c*14;
}

function calcConnPriceWithRates(c, tp) {
  if (c <= 0) return 0; if (c === 1) return tp[1]; if (c <= 10) return 290;
  if (c <= 25) return c*tp[10]; if (c <= 50) return c*tp[25]; if (c <= 100) return c*tp[50];
  if (c <= 200) return c*tp[100]; if (c <= 300) return c*tp[200]; return c*tp[300];
}
...
const Card = ({ children, className, group, style }) => (
  <div className={"rounded-xl shadow p-4 "+(className||"")} style={{...(GROUP_BG[group]||GROUP_BG.default),...style}}>{children}</div>
);
...
const Toggle = ({ label, checked, onChange, hint }) => (
  <label className="flex items-start gap-3 p-3 rounded-lg cursor-pointer border transition mb-2"
    ...
    <input type="checkbox" checked={checked} onChange={e=>onChange(e.target.checked)} className="mt-0.5 w-4 h-4 flex-shrink-0" style={{accentColor:R.primary}} />
    <div>
...
export default function App() {
    ...
    Papa.parse(file, {
      ...
      complete: ({ data: rows }) => {
        const cleaned = rows.map(r => {
          ...
          return {
            id: col(r,"Customer ID"), months, connections: conns,
            tier: getTier(conns), billing,
            mrr: getNum(r,"MRR"),
            reportsPerMonth:       getNum(r,"Reports Created")/months,
            dashPerMonth:          getNum(r,"Dashboards Created")/months,
            downloadsPerMonth:     getNum(r,"Report Downloads")/months,
            dashPublishedPerMonth: getNum(r,"Dashboards Published")/months,
            templatesPerMonth:     getNum(r,"Templates Created")/months,
  ...
  const u = (c) => snapMode ? {
    ...
    reportsPerMonth:       c.reportsPerMonth,
    dashPerMonth:          c.dashPerMonth,
    downloadsPerMonth:     c.downloadsPerMonth,
    dashPublishedPerMonth: c.dashPublishedPerMonth,
    templatesPerMonth:     c.templatesPerMonth,
    budgetsProPerMonth:    c.budgetsProPerMonth,
  ...
  const calcAll = useCallback((customers, s, h, uFn) => {
    ...
    const hybridRows = customers.map(c => {const b=calcHybridOne({...c,...uFn(c)},h,s);return{mrr:toMrr(b,c.billing),cash:toMrr(b,c.billing)*12};});
    return {
  ...
  // Reconciliation rows — includes Hybrid MRR per customer
  const reconRows = useMemo(() => {
    ...
    return data.map(r => {
      ...
      const hybridBase = calcHybridOne({...r,...uc}, hybrid, settings);
      const hybridMrr  = r.billing==="annual"?hybridBase*disc:hybridBase;
      ...
      return {...r, calcMrr, hybridMrr, diff, hybridDiff, diffPct, hybridDiffPct};
    });
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{background:"linear-gradient(135deg,#023E8A 0%,#00B4D8 100%)"}}>
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
        ...
        <button onClick={()=>{if(pw===PASSWORD)setAuthed(true);else setPwErr(true);}}
          className="w-full text-white rounded-lg py-2.5 text-sm font-semibold"
          style={{background:R.primary}}>Access Tool</button>
      </div>
    </div>
  );
  ...
  if (!data) return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{background:"linear-gradient(135deg,#023E8A 0%,#00B4D8 100%)"}}>
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        <div onClick={()=>fileRef.current.click()} className="rounded-xl p-8 text-center cursor-pointer transition mb-4"
    ...
    <div className="min-h-screen" style={{background:"#f8fafc"}}>
      ...
      <div className="text-white px-6 py-4 flex items-center justify-between" style={{background:"linear-gradient(90deg,#023E8A 0%,#0096B7 100%)"}}>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={()=>setSnapMode(m=>!m)} className="px-3 py-1.5 rounded-lg text-xs font-semibold transition"
            style={snapMode?{background:"#fff",color:R.primaryText}:{background:"rgba(255,255,255,0.15)",color:"#fff"}}>
            {snapMode?"📸 Feb Snapshot":"📊 Lifetime Avg"}
          </button>
        ...
        {tab==="settings" && (
          <div className="space-y-8">
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {[["#6366f1","🔗 Connections"],[R.primary,"📄 Reports"],["#10b981","📊 Dashboards"],["#f59e0b","📋 Templates"],["#f43f5e","💰 Budgets Pro"],["#0096B7","🏪 Marketplace"]].map(([c,l])=>(
                <span key={l} className="text-xs font-bold px-3 py-1 rounded-full text-white" style={{background:c}}>{l}</span>
              ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <Card><h2 className="font-semibold text-gray-700 mb-3">📅 Annual Discount</h2><Slider label="Annual billing discount" value={settings.annualDiscount} min={0} max={50} step={1} onChange={set("annualDiscount")} suffix="%" hint="Applied to all annual customers" /></Card>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card group="report"><h2 className="font-semibold mb-3" style={{color:R.primaryText}}>Monthly Fee</h2><Slider label="Per report / month" value={settings.perReportMonthly} min={1} max={50} step={0.5} onChange={set("perReportMonthly")} prefix="$" color={R.primary} /></Card>
                <Card group="report"><h2 className="font-semibold mb-3" style={{color:R.primaryText}}>Pay-Per-Use</h2><Slider label="Per report download" value={settings.perReportUse} min={0.10} max={5} step={0.05} onChange={set("perReportUse")} prefix="$" color={R.primary} /></Card>
                <Card group="report"><h2 className="font-semibold mb-3" style={{color:R.primaryText}}>Packs</h2><Slider label="Reports per pack" value={settings.reportPackSize} min={5} max={50} step={5} onChange={set("reportPackSize")} suffix=" reports" color={R.primary} /><Slider label="Pack price" value={settings.reportPackPrice} min={10} max={200} step={5} onChange={set("reportPackPrice")} prefix="$" color={R.primary} /></Card>
              </div>
            </div>
            <div>
              <GroupDivider label="📊 Dashboards" color="#10b981" />
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card group="dashboard"><h2 className="font-semibold mb-3" style={{color:"#065f46"}}>Monthly Fee</h2><Slider label="Per dashboard / month" value={settings.perDashboardMonthly} min={1} max={50} step={0.5} onChange={set("perDashboardMonthly")} prefix="$" color="#10b981" /></Card>
                <Card group="dashboard"><h2 className="font-semibold mb-3" style={{color:"#065f46"}}>Pay-Per-Use</h2><Slider label="Per dashboard view" value={settings.perDashboardUse} min={0.10} max={5} step={0.05} onChange={set("perDashboardUse")} prefix="$" color="#10b981" /></Card>
                <Card group="dashboard"><h2 className="font-semibold mb-3" style={{color:"#065f46"}}>Packs</h2><Slider label="Dashboards per pack" value={settings.dashPackSize} min={5} max={50} step={5} onChange={set("dashPackSize")} suffix=" dashboards" color="#10b981" /><Slider label="Pack price" value={settings.dashPackPrice} min={10} max={200} step={5} onChange={set("dashPackPrice")} prefix="$" color="#10b981" /></Card>
              </div>
            </div>
            <div>
              <GroupDivider label="📋 Templates" color="#f59e0b" />
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card group="template"><h2 className="font-semibold mb-3" style={{color:"#92400e"}}>Monthly Fee</h2><Slider label="Per template / month" value={settings.perTemplateMonthly} min={1} max={100} step={0.5} onChange={set("perTemplateMonthly")} prefix="$" color="#f59e0b" /></Card>
                <Card group="template"><h2 className="font-semibold mb-3" style={{color:"#92400e"}}>Pay-Per-Use</h2><Slider label="Per template use" value={settings.perTemplateUse} min={0.10} max={10} step={0.1} onChange={set("perTemplateUse")} prefix="$" color="#f59e0b" /></Card>
                <Card group="template"><h2 className="font-semibold mb-3" style={{color:"#92400e"}}>Packs</h2><Slider label="Templates per pack" value={settings.templatePackSize} min={1} max={20} step={1} onChange={set("templatePackSize")} suffix=" templates" color="#f59e0b" /><Slider label="Pack price" value={settings.templatePackPrice} min={10} max={300} step={5} onChange={set("templatePackPrice")} prefix="$" color="#f59e0b" /></Card>
              </div>
            </div>
            <div>
              <GroupDivider label="💰 Budgets Pro" color="#f43f5e" />
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <Card group="budgets"><h2 className="font-semibold mb-3" style={{color:"#9f1239"}}>Monthly Fee</h2><Slider label="Budgets Pro / month" value={settings.perBudgetsProMonthly} min={5} max={200} step={1} onChange={set("perBudgetsProMonthly")} prefix="$" color="#f43f5e" /></Card>
                <Card group="budgets"><h2 className="font-semibold mb-3" style={{color:"#9f1239"}}>Pay-Per-Use</h2><Slider label="Per Budgets Pro use" value={settings.perBudgetsProUse} min={0.50} max={20} step={0.5} onChange={set("perBudgetsProUse")} prefix="$" color="#f43f5e" /></Card>
                <Card group="budgets"><h2 className="font-semibold mb-3" style={{color:"#9f1239"}}>Packs</h2><Slider label="Uses per pack" value={settings.budgetsProPackSize} min={1} max={20} step={1} onChange={set("budgetsProPackSize")} suffix=" uses" color="#f43f5e" /><Slider label="Pack price" value={settings.budgetsProPackPrice} min={10} max={500} step={5} onChange={set("budgetsProPackPrice")} prefix="$" color="#f43f5e" /></Card>
              </div>
            </div>
            <div>
            ...
            <Card>
              <div className="flex items-center justify-between">
                ...
                <button onClick={()=>{setSettings(DEFAULT_SETTINGS);setHybrid(DEFAULT_HYBRID);}} className="px-4 py-2 rounded-lg text-sm font-semibold bg-rose-50 text-rose-600 hover:bg-rose-100">Reset to Defaults</button>
              </div>
            </Card>
          </div>
        )}
        ...
        {tab==="hybrid" && rev && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="space-y-4">
                <Card>
                  <h2 className="font-semibold text-gray-700 mb-3 text-sm">Hybrid vs Current MRR</h2>
                  <div className="space-y-2">
                    {[
                ...
                <Card>
                  ...
                  {[
                    ...
                    <label key={mode} className="flex items-start gap-3 p-3 rounded-lg cursor-pointer border transition mb-2"
                      ...
                      <input type="radio" name="connMode" value={mode} checked={hybrid.connMode===mode} onChange={()=>setH("connMode")(mode)} className="mt-0.5 w-4 h-4 flex-shrink-0" style={{accentColor:R.primary}} />
                      <div>
                ...
                {[
                  ...
                  <Card key={title}>
                    ...
                    {opts.map(({label,k,off})=>(
                      <Toggle key={k} label={label} checked={hybrid[k]} onChange={v=>{setH(k)(v);if(v)off.forEach(o=>setH(o)(false));}} />
                    ))}
                  </Card>
                ))}
                <Card>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="w-full lg:w-60 sticky top-4 self-start space-y-3">
              <div className="rounded-xl p-4" style={{background:"#FFFBEB",border:"2px solid #FCD34D"}}>
                ...
                <button onClick={()=>setTab("hybrid")} className="w-full py-2 text-white rounded-lg text-xs font-semibold" style={{background:"#f59e0b"}}>Go to Hybrid Builder →</button>
              </div>
            </div>
          </div>
        )}
        ...
        {tab==="reconciliation" && reconTotals && (
          <div className="space-y-5">
            <div className="rounded-xl p-4" style={{background:"#FFFBEB",border:"1px solid #FCD34D"}}>
              ...
              <p className="text-xs text-amber-700">Calc MRR uses connection-based pricing only. Hybrid MRR applies your full Hybrid Builder configuration to each customer individually.</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {reconPageRows.map((r,i)=>{
                      ...
                      return (
                        <tr key={String(r.id)+"-"+i} className="border-b last:border-0 text-xs" style={{background:rowBg}}>
                          ...
                          <td className="px-4 py-2.5 font-bold font-mono" style={{color:isOver?"#e11d48":isUnder?"#b45309":"#9ca3af"}}>{r.diff>=0?"+":""}{fmt(r.diff)}</td>
                          ...
                          <td className="px-4 py-2.5">
                            {r.hybridDiffPct!==null
                              ? <span className={"px-2 py-0.5 rounded font-semibold text-xs "+(Math.abs(r.hybridDiffPct)<1?"bg-gray-100 text-gray-500":hybridUp?"bg-green-100 text-green-700":"bg-rose-100 text-rose-700")}>{r.hybridDiffPct>=0?"+":""}{r.hybridDiffPct.toFixed(1)}%</span>
                              : "—"}
                          </td>
                        </tr>
                      );
                    ...
                    {reconPageRows.length===0&&(
                      <tr><td colSpan="9" className="px-4 py-8 text-center text-gray-400 text-sm">No customers match this filter</td></tr>
                    )}
                  </tbody>
              ...
              <div className="flex items-center justify-between text-xs text-gray-500">
                ...
                <div className="flex gap-1">
                  ...
                  {Array.from({length:Math.min(5,reconTotalPages)}).map((_,i)=>{
                    ...
                    return <button key={p} onClick={()=>setReconPage(p)} className="px-2.5 py-1 rounded border text-xs"
                      style={p===reconPage?{background:R.primary,color:"#fff",borderColor:R.primary}:{}}>{p+1}</button>;
                  })}
